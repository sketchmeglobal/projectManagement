    ...
    public function ajax_project_report_table_data() {
        ...
        if(sizeof($p_data) > 0){
            ...
            foreach($p_data as $index => $val){
                ...
                $quotationDetail = array();
                $quotationDetail = $p_description->quotationDetail;
                //echo json_encode($quotationDetail);
                ...
                //if($p_description->projectDetail->projectAutoGeneratedId == '20231'){
                    //echo ' ID: '. $p_description->projectDetail->projectAutoGeneratedId;
                    if(sizeof($quotationDetail) >= 1){
                        //echo ' 1. ID: '. $p_description->projectDetail->projectAutoGeneratedId;
                        //echo ' size: ' . sizeof($quotationDetail);
                        if(isset($quotationDetail[0]->bi_obj)){
                            for($i = 0; $i < sizeof($quotationDetail); $i++){
                                if(isset($quotationDetail[$i]->tax_GrossAmount)){
                                    $grossAmount = $grossAmount + $quotationDetail[$i]->tax_GrossAmount;
                                }

                                if(isset($quotationDetail[$i]->tax_TaxableAmount)){
                                    $taxableAmount = $taxableAmount + $quotationDetail[$i]->tax_TaxableAmount;
                                }

                                if(isset($quotationDetail[$i]->tax_TotalTax)){
                                    $totalTax = $totalTax + $quotationDetail[$i]->tax_TotalTax;
                                }

                                if(isset($quotationDetail[$i]->tax_NetAmount)){
                                    $netAmount = $netAmount + $quotationDetail[$i]->tax_NetAmount;
                                }
                                ...
                                if(isset($quotationDetail[$i]->tax_TaxableAmount)){
                                    $tax_TaxableAmount = $quotationDetail[$i]->tax_TaxableAmount;
                                }
                                if(sizeof($commissions) > 0){
                                    for($j = 0; $j < sizeof($commissions); $j++){
                                        $comi_rate_type = 0;
                                        if(isset($commissions[$j]->comi_rate_type)){
                                            $comi_rate_type = $commissions[$j]->comi_rate_type;
                                        }
                                        
                                        $comi_amount = 0;
                                        if(isset($commissions[$j]->comi_amount)){
                                            $comi_amount = $commissions[$j]->comi_amount;
                                        }
                                        $calculated_commission = 0;

                                        if($comi_rate_type == 1){
                                            $calculated_commission = $comi_amount;
                                        }else{
                                            $calculated_commission = $tax_TaxableAmount * $comi_amount * 0.01;
                                        }//end if

                                        $commission = $commission + $calculated_commission;
                                    }//end for
                                }//end if
                ...
                $mcosts = array();
                $misc_amount = 0;
                if(isset($p_description->mcosts)){
                    $mcosts = $p_description->mcosts;
                    if(sizeof($mcosts) > 0){
                        for($k = 0; $k < sizeof($mcosts); $k++){
                            $misc_amount = $misc_amount + $mcosts[$k]->misc_amount;
                        }//end for
                    }//end if
                }//end if
                ...
                $nestedData['grossAmount'] = number_format($grossAmount, 2);
                $nestedData['taxableAmount'] = number_format($taxableAmount, 2);
                $nestedData['totalTax'] = number_format($totalTax, 2);
                $nestedData['netAmount'] = number_format($netAmount, 2);
                $nestedData['commission'] = number_format($commission, 2);
                $nestedData['otherExpanses'] = number_format($misc_amount, 2);
                $nestedData['comments'] = '';
                ...
                $taxableAmountTotal = $taxableAmountTotal + $taxableAmount;
                $totalTaxTotal = $totalTaxTotal + $totalTax;
                $netAmountTotal = $netAmountTotal + $netAmount;
                $commissionTotal = $commissionTotal + $commission;
                $otherExpansesTotal = $otherExpansesTotal + $misc_amount;
            }//end foreach
            ...
            $nestedData['projectStatus'] = 'Total';
            $nestedData['grossAmount'] = number_format($grossAmountTotal, 2);
            $nestedData['taxableAmount'] = number_format($taxableAmountTotal, 2);
            $nestedData['totalTax'] = number_format($totalTaxTotal, 2);
            $nestedData['netAmount'] = number_format($netAmountTotal, 2);
            $nestedData['commission'] = number_format($commissionTotal, 2);
            $nestedData['otherExpanses'] = number_format($otherExpansesTotal, 2);
            $nestedData['comments'] = '';
