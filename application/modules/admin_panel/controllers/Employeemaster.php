...
class Employeemaster extends My_Controller {
    ...
    public function employeemaster_managemnt(){        
        if($this->check_permission(array()) == true) {
            $this->load->model('Employeemaster_m');
            $data = $this->Employeemaster_m->employeemaster_managemnt();
    ...
    public function ajax_unique_username(){
        ...
        if($this->check_permission(array()) == true) {
            $this->load->model('Employeemaster_m');
            $data = $this->Employeemaster_m->ajax_unique_username();
            echo json_encode($data, JSON_HEX_QUOT | JSON_HEX_TAG);
            exit();
        }        
    ...
    public function acc_master_on_usertype(){
        ...
        if($this->check_permission(array()) == true) {
            $this->load->model('Employeemaster_m');
            $data = $this->Employeemaster_m->acc_master_on_usertype();
            echo json_encode($data, JSON_HEX_QUOT | JSON_HEX_TAG);
            exit();
        }
    ...
    public function ajax_unique_username_edit(){
        ...
        if($this->check_permission(array()) == true) {
            $this->load->model('Employeemaster_m');
            $data = $this->Employeemaster_m->ajax_unique_username_edit();
            echo json_encode($data, JSON_HEX_QUOT | JSON_HEX_TAG);
            exit();
        }
